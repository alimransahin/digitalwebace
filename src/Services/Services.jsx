import React from "react";
import "./Services.css";
import ScrollAnimation from "react-animate-on-scroll";
const Services = () => {
  return (
    <div className="bg-[url('https://images.unsplash.com/photo-1482977036925-e8fcaa643657?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80')] bg-cover bg-no-repeat px-10 py-16 ">
      <div className="title text-center text-2xl font-bold text-blue-500 py-3">
        Our Services
      </div>
      <div className="services grid md:grid-cols-3 sm:grid-cols-2 gap-5 ">
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>

            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>
            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>{" "}
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>
            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>{" "}
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>
            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>{" "}
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>
            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>{" "}
        <ScrollAnimation animateIn="fadeInUpBig">
          <div className="card bg-gray-100 p-2 rounded-xl hover:scale-105 duration-300">
            <div className="img text-center ">
              <img
                className="inline-block w-24"
                src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRvQYKJ0sAZ-di6z4hZgr3nwiw9iVYBB-dgqUmhgcdrmh9tXSODmIlS8L5p2K5rosT1YkA&usqp=CAU"
                alt="ui/ux"
              />
            </div>
            <div className="card-title text-blue-400 text-center font-medium text-lg">
              WEB DESIGN
            </div>
            <div className="card-body text-center">
              Lorem, ipsum dolor sit amet consectetur adipisicing elit.
              Inventore, veritatis necessitatibus. Perspiciatis mollitia
              distinctio hic blanditiis quisquam culpa vitae sint!
            </div>
          </div>
        </ScrollAnimation>
      </div>
    </div>
  );
};

export default Services;
